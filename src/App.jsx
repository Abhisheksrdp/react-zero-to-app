import downloadImg from "./assets/download.png";
import Kfc from "./assets/kfc.jpeg";
import Pizza from './assets/Pizza.webp'

// header
//   - logo
//   - Nav menu
// body
//    -serach
//    -restro-cards
// footer
//    -copyright
//    -links

const Hydrestaurents = [
  {
    id: "r1",
    restName: "Pasta Palace",
    dishTitle: "Spaghetti Carbonara",
    image: "https://images.pexels.com/photos/1279330/pexels-photo-1279330.jpeg",
    cuisines: ["Italian", "Pasta"],
    rating: 4.5,
    waitTime: 30
  },
  {
    id: "r2",
    restName: "Burger Binge",
    dishTitle: "Classic Cheeseburger",
    image: "https://images.pexels.com/photos/1639566/pexels-photo-1639566.jpeg",
    cuisines: ["American", "Fast Food"],
    rating: 4.2,
    waitTime: 25
  },
  {
    id: "r3",
    restName: "Taco Town",
    dishTitle: "Beef Taco",
    image: "https://images.pexels.com/photos/461198/pexels-photo-461198.jpeg",
    cuisines: ["Mexican", "Street Food"],
    rating: 4.3,
    waitTime: 20
  },
  {
    id: "r4",
    restName: "Sushi Central",
    dishTitle: "Salmon Sushi",
    image: "https://images.pexels.com/photos/357756/pexels-photo-357756.jpeg",
    cuisines: ["Japanese", "Seafood"],
    rating: 4.7,
    waitTime: 35
  },
  {
    id: "r5",
    restName: "Curry Corner",
    dishTitle: "Chicken Biryani",
    image: "https://images.pexels.com/photos/461198/pexels-photo-461198.jpeg",
    cuisines: ["Indian", "Rice"],
    rating: 4.6,
    waitTime: 40
  },
  {
    id: "r6",
    restName: "Grill Garden",
    dishTitle: "Grilled Chicken",
    image: "https://images.pexels.com/photos/616354/pexels-photo-616354.jpeg",
    cuisines: ["American", "Barbecue"],
    rating: 4.4,
    waitTime: 28
  },
  {
    id: "r7",
    restName: "Vegan Vibes",
    dishTitle: "Vegan Salad",
    image: "https://images.pexels.com/photos/1640777/pexels-photo-1640777.jpeg",
    cuisines: ["Vegan", "Healthy"],
    rating: 4.8,
    waitTime: 22
  },
  {
    id: "r8",
    restName: "Pizza Point",
    dishTitle: "Margherita Pizza",
    image: "https://images.pexels.com/photos/724216/pexels-photo-724216.jpeg",
    cuisines: ["Italian", "Fast Food"],
    rating: 4.5,
    waitTime: 30
  },
  {
    id: "r9",
    restName: "Noodle Nest",
    dishTitle: "Beef Chow Mein",
    image: "https://images.pexels.com/photos/128388/pexels-photo-128388.jpeg",
    cuisines: ["Chinese", "Noodles"],
    rating: 4.3,
    waitTime: 26
  },
  {
    id: "r10",
    restName: "Spice Hub",
    dishTitle: "Chicken Tikka Masala",
    image: "https://images.pexels.com/photos/461198/pexels-photo-461198.jpeg",
    cuisines: ["Indian", "Curry"],
    rating: 4.7,
    waitTime: 33
  },
  {
    id: "r11",
    restName: "Pasta Palace",
    dishTitle: "Penne Arrabbiata",
    image: "https://images.pexels.com/photos/1279330/pexels-photo-1279330.jpeg",
    cuisines: ["Italian", "Pasta"],
    rating: 4.4,
    waitTime: 29
  },
  {
    id: "r12",
    restName: "Burger Binge",
    dishTitle: "Veggie Burger",
    image: "https://images.pexels.com/photos/1352276/pexels-photo-1352276.jpeg",
    cuisines: ["Vegetarian", "Fast Food"],
    rating: 4.2,
    waitTime: 24
  },
  {
    id: "r13",
    restName: "Taco Town",
    dishTitle: "Fish Taco",
    image: "https://images.pexels.com/photos/461198/pexels-photo-461198.jpeg",
    cuisines: ["Seafood", "Mexican"],
    rating: 4.3,
    waitTime: 23
  },
  {
    id: "r14",
    restName: "Sushi Central",
    dishTitle: "Tuna Roll",
    image: "https://images.pexels.com/photos/357756/pexels-photo-357756.jpeg",
    cuisines: ["Japanese", "Seafood"],
    rating: 4.6,
    waitTime: 34
  },
  {
    id: "r15",
    restName: "Curry Corner",
    dishTitle: "Paneer Butter Masala",
    image: "https://images.pexels.com/photos/461198/pexels-photo-461198.jpeg",
    cuisines: ["Vegetarian", "Curry"],
    rating: 4.5,
    waitTime: 32
  },
  {
    id: "r16",
    restName: "Grill Garden",
    dishTitle: "BBQ Ribs",
    image: "https://images.pexels.com/photos/616354/pexels-photo-616354.jpeg",
    cuisines: ["Barbecue", "American"],
    rating: 4.5,
    waitTime: 29
  },
  {
    id: "r17",
    restName: "Vegan Vibes",
    dishTitle: "Quinoa Salad",
    image: "https://images.pexels.com/photos/1640777/pexels-photo-1640777.jpeg",
    cuisines: ["Vegan", "Healthy"],
    rating: 4.7,
    waitTime: 21
  },
  {
    id: "r18",
    restName: "Pizza Point",
    dishTitle: "Pepperoni Pizza",
    image: "https://images.pexels.com/photos/724216/pexels-photo-724216.jpeg",
    cuisines: ["Italian", "Fast Food"],
    rating: 4.6,
    waitTime: 31
  },
  {
    id: "r19",
    restName: "Noodle Nest",
    dishTitle: "Chicken Hakka Noodles",
    image: "https://images.pexels.com/photos/128388/pexels-photo-128388.jpeg",
    cuisines: ["Chinese", "Noodles"],
    rating: 4.2,
    waitTime: 28
  },
  {
    id: "r20",
    restName: "Spice Hub",
    dishTitle: "Lamb Rogan Josh",
    image: "https://images.pexels.com/photos/461198/pexels-photo-461198.jpeg",
    cuisines: ["Indian", "Curry"],
    rating: 4.8,
    waitTime: 38
  },
  {
    id: "r21",
    restName: "Pasta Palace",
    dishTitle: "Lasagna",
    image: "https://images.pexels.com/photos/1279330/pexels-photo-1279330.jpeg",
    cuisines: ["Italian", "Pasta"],
    rating: 4.5,
    waitTime: 30
  },
  {
    id: "r22",
    restName: "Burger Binge",
    dishTitle: "Double Cheeseburger",
    image: "https://images.pexels.com/photos/1639566/pexels-photo-1639566.jpeg",
    cuisines: ["American", "Fast Food"],
    rating: 4.3,
    waitTime: 26
  },
  {
    id: "r23",
    restName: "Taco Town",
    dishTitle: "Chicken Quesadilla",
    image: "https://images.pexels.com/photos/461198/pexels-photo-461198.jpeg",
    cuisines: ["Mexican", "Cheese"],
    rating: 4.4,
    waitTime: 27
  },
  {
    id: "r24",
    restName: "Sushi Central",
    dishTitle: "Avocado Roll",
    image: "https://images.pexels.com/photos/357756/pexels-photo-357756.jpeg",
    cuisines: ["Japanese", "Vegetarian"],
    rating: 4.6,
    waitTime: 30
  },
  {
    id: "r25",
    restName: "Curry Corner",
    dishTitle: "Lentil Dal",
    image: "https://images.pexels.com/photos/461198/pexels-photo-461198.jpeg",
    cuisines: ["Indian", "Vegetarian"],
    rating: 4.5,
    waitTime: 35
  },
  {
    id: "r26",
    restName: "Grill Garden",
    dishTitle: "Grilled Salmon",
    image: "https://images.pexels.com/photos/616354/pexels-photo-616354.jpeg",
    cuisines: ["Seafood", "Barbecue"],
    rating: 4.6,
    waitTime: 32
  },
  {
    id: "r27",
    restName: "Vegan Vibes",
    dishTitle: "Fruit Bowl",
    image: "https://images.pexels.com/photos/1640777/pexels-photo-1640777.jpeg",
    cuisines: ["Vegan", "Healthy"],
    rating: 4.8,
    waitTime: 20
  },
  {
    id: "r28",
    restName: "Pizza Point",
    dishTitle: "Veggie Pizza",
    image: "https://images.pexels.com/photos/724216/pexels-photo-724216.jpeg",
    cuisines: ["Vegetarian", "Fast Food"],
    rating: 4.4,
    waitTime: 29
  },
  {
    id: "r29",
    restName: "Noodle Nest",
    dishTitle: "Veg Hakka Noodles",
    image: "https://images.pexels.com/photos/128388/pexels-photo-128388.jpeg",
    cuisines: ["Chinese", "Vegetarian"],
    rating: 4.2,
    waitTime: 27
  },
  {
    id: "r30",
    restName: "Spice Hub",
    dishTitle: "Dal Makhani",
    image: "https://images.pexels.com/photos/461198/pexels-photo-461198.jpeg",
    cuisines: ["Indian", "Curry"],
    rating: 4.7,
    waitTime: 36
  }
];



const Restrocard = ({id, restName, dishTitle,image,cuisines,rating, waitTime}) => {
  return(

    <div className="restaurent-container" >

          <div className="restro-card">

            <div className="restro-card-container">

              <img src={image} alt={restName} />
              <h2>title : {dishTitle}</h2>
              <h3>cuisines: {Array.isArray(cuisines) ? cuisines.join(", ") : cuisines || "N/A"}</h3>
              <h4>ratings: {rating}</h4>
              <h4>delivery time : {waitTime}</h4>

            </div>

          </div>
        </div>
  );
}

const Header = () => {
  return (
    <div className="header">
      <div className="header-container">
        <div>
          <img src={downloadImg} alt="logo" />
        </div>

        <div className="list-container">
          <ul>
            <li>Home</li>
            <li>About Us</li>
            <li>Services</li>
            <li>Contact</li>
            <li>Cart</li>
          </ul>
        </div>
      </div>
    </div>
  );
};

const Body = () => {
  return (
    <div className="body">
      <div className="input-container">
        <input type="text" style={{ border: "1px solid black" }} />
        <button>Search</button>
      </div>

      
      <div className="restaurents"  style={{ width: '100%',  border: '2px solid red', minHeight: '500px', display : 'flex', flexWrap:'wrap', justifyContent :'space-evenly' }} >
        
       { Hydrestaurents.map((restro) => 
       <Restrocard 
        
       key = {restro.id}
       id ={restro.id}
       restName={restro.restName}
       image = {restro.image}
       dishTitle = {restro.dishTitle}
       cuisines = {restro.cuisines}
       rating = {restro.rating}
       waitTime = {restro.waitTime}
       


       />)
        }
        

      </div>
    </div>

    
  );
};

const App = () => {
  return (
    <div className="app">
      <Header />
      <Body />
    </div>
  );
};

export default App;
